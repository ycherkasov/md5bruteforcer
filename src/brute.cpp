...
Examples:
Hash for '12345'
3 5 48-57 827ccb0eea8a706c4c34a16891f84e7b

Hash for '99999'
3 5 48-57 d3eb9a9233e52948740d7eb8c3062d14

Hash for 'querty'
3 6 97-122 356d0f282e7da14cb73f2614d191933b

Hash for 'password'
3 6 97-122 5f4dcc3b5aa765d61d8327deb882cf99
*/
...
string compose_charset(const char* ranges_str){
    ...
    string symbols;
    symbols.reserve(255);
    tokenize(ranges_str, ':', ranges);

    // append symbols from every range
    for(auto &range : ranges){
        vector<string> minmax;
        tokenize(range, '-', minmax);
        if (minmax.size() != 2){
            usage_exit();
        }
        // fetch range from string
		size_t min_range = string_to_any<size_t>(minmax[0]);
		size_t max_range = string_to_any<size_t>(minmax[1]);
...
int main(int argc, char* argv[]){
	...
	if ((32 != hash_to_check.size()) 
		|| 32 != (count_if(hash_to_check.begin(), hash_to_check.end(), std::isxdigit))) {
    ...
    try {
		min_length = string_to_any<size_t>(argv[min_length_param]);
		max_length = string_to_any<size_t>(argv[max_length_param]);
		charsets = compose_charset(argv[charset_param]);
    }
	catch (const string_cast_error& e) {
        cerr << e.what() << endl;
        usage_exit();
    }
